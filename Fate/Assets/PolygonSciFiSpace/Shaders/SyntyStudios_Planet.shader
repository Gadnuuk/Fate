...
Shader "SyntyStudios/Planets_URP"
{
    Properties
    {
        [Header(Outline)]
        ...
        _PlanetOverlayTop("Planet Overlay Top", 2D) = "white" {}
        _PlanetSurfaceContrast("PlanetSurfaceContrast", Float) = 0
        _Planet_Tiling("Planet_Tiling", Float) = 1
        _Color0("Color 0", Color) = (0,0,0,0)
        _Second_Color("Second_Color", Color) = (0,0,0,0)
        _Planet_Falloff("Planet_Falloff", Float) = 2.2

        [Header(Clouds)]
        _Cloud_Contrast("Cloud_Contrast", Float) = 0
        _Clouds("Clouds", 2D) = "white" {}
        _CloudPower("Cloud Power", Range(1,256)) = 1
        _CloudColor("Cloud Color", Color) = (0,0,0,0)
        _CloudCoverage("Cloud Coverage", Float) = 0
        _CloudSpeed("Cloud Speed", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings OutlineVertex(Attributes IN)
            {
                ...
                float3 posOS = IN.positionOS.xyz * (1.0 + _ASEOutlineWidth);
            ...
            {
                return half4(_ASEOutlineColor.rgb, 1.0h);
            }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            TEXTURE2D(_PlanetOverlayTop);      SAMPLER(sampler_PlanetOverlayTop);
            TEXTURE2D(_PlanetOverlayMiddle);   SAMPLER(sampler_PlanetOverlayMiddle);
            TEXTURE2D(_Clouds);                SAMPLER(sampler_Clouds);
            TEXTURE2D(_Normals);               SAMPLER(sampler_Normals);

            CBUFFER_START(UnityPerMaterial)
                float4 _PlanetOverlayTop_ST;
                float4 _PlanetOverlayMiddle_ST;
                float4 _Clouds_ST;
                float4 _Normals_ST;
                ...
                float _PlanetSurfaceContrast;
                float _Planet_Tiling;
                float _Planet_Falloff;

                float _Cloud_Contrast;
                float _CloudPower;
            ...
            float4 CalculateContrast(float contrastValue, float4 colorTarget)
            {
                ...
                return mul(m, colorTarget);
            }

            float4 TriplanarSamplingCF(
                Texture2D topTexMap, SamplerState topSampler,
                Texture2D midTexMap, SamplerState midSampler,
                Texture2D botTexMap, SamplerState botSampler,
                float3 objectPos,
                float3 objectNormal,
                float falloff,
                float tiling)
            {
                float3 projNormal = pow(abs(objectNormal), falloff);
                projNormal /= max(projNormal.x + projNormal.y + projNormal.z, 0.00001);

                float3 nsign = sign(objectNormal);
                float negProjNormalY = max(0.0, projNormal.y * -nsign.y);
                projNormal.y = max(0.0, projNormal.y * nsign.y);

                float4 xTex  = topTexMap.Sample(midSampler, tiling * objectPos.zy * float2(nsign.x, 1.0));
                float4 yTex  = topTexMap.Sample(topSampler, tiling * objectPos.xz * float2(nsign.y, 1.0));
                float4 yTexN = botTexMap.Sample(botSampler, tiling * objectPos.xz * float2(nsign.y, 1.0));
                float4 zTex  = midTexMap.Sample(midSampler, tiling * objectPos.xy * float2(-nsign.z, 1.0));

                return xTex * projNormal.x + yTex * projNormal.y + yTexN * negProjNormalY + zTex * projNormal.z;
            }
            ...
            Varyings LitVertex(Attributes IN)
            {
                ...
                float3x3 tbn = float3x3(normInputs.tangentWS, normInputs.bitangentWS, normInputs.normalWS);
                OUT.viewDirTS = mul(transpose(tbn), viewDirWS);
            ...
            half4 LitFragment(Varyings IN) : SV_Target
            {
                ...
                // Clouds
                // ----------------------------
                float2 cloudUV = uv + float2(1.0, 0.0);
                cloudUV += _Time.y * float2(_CloudSpeed, 0.0);

                float posterizeSteps = max(1.0, _CloudPower);
                float div = 256.0 / posterizeSteps;
                float4 cloudSample = SAMPLE_TEXTURE2D(_Clouds, sampler_Clouds, cloudUV);
                float4 posterizedCloud = floor(cloudSample * div) / div;

                float4 cloudLerp = lerp(_CloudColor, float4(0,0,0,0), posterizedCloud);
                cloudLerp = lerp(cloudLerp, float4(0,0,0,0), IN.color.r);

                float3 cloudContrast = CalculateContrast(_CloudCoverage, cloudLerp).rgb;
                float cloudGray = dot(cloudContrast, float3(0.299, 0.587, 0.114));
                float3 cloudFinal = lerp(cloudContrast, cloudGray.xxx, 0.0);

                // ----------------------------
                // Planet triplanar
                // ----------------------------
                float4 triplanarA = TriplanarSamplingCF(
                    _PlanetOverlayTop, sampler_PlanetOverlayTop,
                    _PlanetOverlayMiddle, sampler_PlanetOverlayMiddle,
                    _PlanetOverlayTop, sampler_PlanetOverlayTop,
                    IN.positionOS,
                ...
                float4 baseColorLerp = lerp(_Color0, _Second_Color, CalculateContrast(_PlanetSurfaceContrast, triplanarA));

                float4 triplanarB = TriplanarSamplingCF(
                    _PlanetOverlayTop, sampler_PlanetOverlayTop,
                    _PlanetOverlayMiddle, sampler_PlanetOverlayMiddle,
                    _PlanetOverlayTop, sampler_PlanetOverlayTop,
                    IN.positionOS,
                ...
                float4 cloudTintLerp = lerp(_Color0, _Second_Color, CalculateContrast(_Cloud_Contrast, triplanarB) * -1.0);
                float4 combinedPlanet = clamp(baseColorLerp + cloudTintLerp, float4(0,0,0,0), float4(1,1,1,0));

                float3 albedo = ScreenBlend(cloudFinal, combinedPlanet.rgb);

                // ----------------------------
                // Rim
                // ----------------------------
                float2 normalUV = uv * _Normals_ST.xy + _Normals_ST.zw;
                float3 normalTS = UnpackNormal(SAMPLE_TEXTURE2D(_Normals, sampler_Normals, normalUV));
                float3 viewDirTS = normalize(IN.viewDirTS);
                float rimDot = dot(normalTS, viewDirTS);
                float3 rimEmission = pow(1.0 - saturate(rimDot), _RimPower) * _RimColor.rgb;
                ...
                surfaceData.normalTS = half3(0, 0, 1);   // keep lighting based on mesh normals
                surfaceData.emission = rimEmission;
