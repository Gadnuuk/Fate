...
public class BodyTurnHandler : MonoBehaviour
{
    ...
    [SerializeField] private Transform viewOrienter;
    ...
    [SerializeField] private float _angle; // body rotation angle
    public float Angle
    {
        get => _angle;
        set
    ...
    private void Update()
    {
        if (momentaryTurn)
        {
            body.rotation = Quaternion.Euler(0, viewOrienter.rotation.eulerAngles.y, 0);
        }
        else
        {
            // body rotation calculation
            Angle = Vector3.Angle(body.right, Quaternion.Euler(0, viewOrienter.rotation.eulerAngles.y, 0) * Vector3.forward);
        }
    }
    ...
    IEnumerator BodyTurn()
    {
        ...
        while (t < turnTime)
        {
            ...
            body.rotation = Quaternion.Slerp(startRot, Quaternion.Euler(0, viewOrienter.rotation.eulerAngles.y, 0), t / turnTime);
